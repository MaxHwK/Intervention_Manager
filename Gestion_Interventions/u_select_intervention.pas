...
{$R *.lfm}
 ...
 uses u_feuille_style, u_list_intervention, u_modele;
...
begin
   ...
   if  pnl_tous_edit.Visible  then
    f_list_intervention.affi_data(modele.intervention_liste_tous)
else if  pnl_num_edit.visible  then
     f_list_intervention.affi_data(modele.intervention_liste_num(edt_num.text))
else if  pnl_periode_edit.visible  then
     f_list_intervention.affi_data(modele.intervention_liste_periode(DateToStr(edt_dtdeb.date)
				,DateToStr(edt_dtfin.date)))
else if  pnl_client_edit.visible  then
     f_list_intervention.affi_data(modele.intervention_liste_client(edt_num_cl.text, edt_nom_cl.text))
else if  pnl_intervenant_edit.visible  then
     f_list_intervention.affi_data(modele.intervention_liste_interv(edt_num_int.text,edt_nom_int.text))
else if pnl_contrat_edit.visible  then
     f_list_intervention.affi_data(modele.intervention_liste_cont(edt_cont.text))
end;
