...
uses   u_feuille_style, u_facture_list, u_modele, u_loaddataset;

var
  flux_facture: TLoadDataSet;
...
begin
   ...
   cbo_pres_code.Clear;
   cbo_pres_type.clear;
   flux_facture := modele.facture_tous;
end;
...
begin
   init;
   cbo_pres_code.items := flux_facture.Column('code');
   cbo_pres_type.items := flux_facture.Column('type');
...
begin
   if cbo.ItemIndex > -1
   then begin
          flux_facture.Position (cle,cbo.items[cbo.itemindex]);
          // T := flux_facture.Read;
          // f_facture_list.line_add(['code',T.Valeur['code'],'type',T.Valeur['type'];
          f_facture_list.line_add(flux_facture.Read);
